# @copyright https://github.com/AprendeIngenia
...
# Nos ayuda a preprocesar las imagenes que le entreguemos al modelo
from tensorflow.python.keras.preprocessing.image import ImageDataGenerator
# Optimizador con el que vamos a entrenar el modelo
from tensorflow.python.keras import optimizers
# Nos permite hacer redes neuronales secuenciales
from tensorflow.python.keras.models import Sequential
from tensorflow.python.keras.layers import Dropout, Flatten, Dense, Activation
# Capas para hacer las convoluciones
from tensorflow.python.keras.layers import Convolution2D, MaxPooling2D
# Si hay una sesion de keras, lo cerramos para tener todo limpio
...
cnn.add(Convolution2D(filtrosconv1, tam_filtro1, padding='same', input_shape=(
    altura, longitud, 3), activation='relu'))  # Agregamos la primera capa
# Es una convolucion y realizamos config
# Despues de la primera capa vamos a tener una capa de max pooling y asignamos el tamaño
cnn.add(MaxPooling2D(pool_size=tam_pool))

cnn.add(Convolution2D(filtrosconv2, tam_filtro2, padding='same',
        activation='relu'))  # Agregamos nueva capa
...
# Apagamos el 50% de las neuronas en la funcion anterior para no sobreajustar la red
cnn.add(Dropout(0.5))
# Es nuestra ultima capa, es la que nos dice la probabilidad de que sea alguna de las clases
...
cnn.compile(loss='categorical_crossentropy',
            optimizer=optimizar, metrics=['accuracy'])
